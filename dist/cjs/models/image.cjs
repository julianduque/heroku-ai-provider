...
exports.HerokuImageModel = void 0;
...
class HerokuImageModel {
    ...
    validateConstructorParameters(model, apiKey, baseUrl) {
        ...
        if (!apiKey || typeof apiKey !== "string" || apiKey.trim().length === 0) {
            throw (0, error_handling_js_1.createValidationError)("apiKey must be a non-empty string", "apiKey", "[REDACTED]");
        }
        if (!baseUrl ||
            typeof baseUrl !== "string" ||
            baseUrl.trim().length === 0) {
            throw (0, error_handling_js_1.createValidationError)("baseUrl must be a non-empty string", "baseUrl", baseUrl);
        }
        try {
            const parsed = new URL(baseUrl);
            if (!["http:", "https:"].includes(parsed.protocol)) {
                throw (0, error_handling_js_1.createValidationError)("baseUrl must use HTTP or HTTPS protocol", "baseUrl", baseUrl);
            }
        }
        catch (error) {
            if (error instanceof Error && error.name === "TypeError") {
                throw (0, error_handling_js_1.createValidationError)(`baseUrl is not a valid URL: ${error.message}`, "baseUrl", baseUrl);
            }
            throw error;
        }
    }
    async doGenerate(options) {
        ...
        if (!options.prompt || options.prompt.trim().length === 0) {
            throw (0, error_handling_js_1.createValidationError)("prompt must be a non-empty string", "prompt", options.prompt);
        }
        if (!Number.isInteger(options.n) || options.n <= 0) {
            throw (0, error_handling_js_1.createValidationError)("n must be a positive integer", "n", options.n);
        }
        ...
        if (options.size) {
            ...
            if (!sizePattern.test(options.size)) {
                throw (0, error_handling_js_1.createValidationError)("size must be in the format {width}x{height}", "size", options.size);
            }
        }
        const baseRequest = {
            model: this.model,
...
exports.HerokuImageModel = HerokuImageModel;
