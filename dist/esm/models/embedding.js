...
export class HerokuEmbeddingModel {
    ...
    async doEmbed(options) {
        ...
        const hasEmptyStrings = inputArray.some((text) => !text || typeof text !== "string" || text.trim().length === 0);
        if (hasEmptyStrings) {
            throw new APICallError({
     ...
     * ```
     */
    async embedBatch(texts, chunkSize = this.maxEmbeddingsPerCall) {
 ...
 * ```
 */
export function createEmbedFunction(model) {
