...
/**
 * Extended options interface that includes tools and toolChoice properties
 * that may be present on the options object but aren't part of the base interface
 */
interface ExtendedCallOptions extends LanguageModelV1CallOptions {
  tools?: ToolInput[];
...
export class HerokuChatLanguageModel implements LanguageModelV1 {
  ...
  async doGenerate(options: LanguageModelV1CallOptions) {
    ...
    try {
      ...
      const extendedOptions = options as ExtendedCallOptions;
      const tools =
        extendedOptions.tools ??
        (options.mode?.type === "regular" ? options.mode.tools : undefined);
      const toolChoice =
        extendedOptions.toolChoice ??
  ...
  private mapPromptToMessages(
    ...
    if (tools && tools.length > 0) {
      const toolDefinitions = this.mapToolsToHerokuFormat(tools)
        .map(
          (tool) =>
            `{"type": "function", "function": ${JSON.stringify(
              tool.function,
            )}}`,
        )
        .join("\n");
  ...
  private mapResponseToOutput(
    ...
    if (
      ...
      finishReason = "tool-calls" as LanguageModelV1FinishReason;
    } else if (finishReasonStr === "content_filter") {
      finishReason = "content-filter" as LanguageModelV1FinishReason;
    } else if (finishReasonStr === "max_tokens") {
      finishReason = "length" as LanguageModelV1FinishReason;
    } else {
      ...
      finishReason = validFinishReasons.includes(finishReasonStr)
        ? (finishReasonStr as LanguageModelV1FinishReason)
        : ("other" as LanguageModelV1FinishReason);
    }
  ...
  private mapChunkToStreamPart(
    ...
    if (!delta) {
      ...
      const finishReason = choice.finish_reason as LanguageModelV1FinishReason;
      if (finishReason) {
        ...
        if (
          ...
          normalizedFinishReason = "tool-calls" as LanguageModelV1FinishReason;
        } else if (finishReasonStr === "content_filter") {
          normalizedFinishReason =
            "content-filter" as LanguageModelV1FinishReason;
        } else if (finishReasonStr === "max_tokens") {
          normalizedFinishReason = "length" as LanguageModelV1FinishReason;
        } else {
    ...
    if (delta.content && typeof delta.content === "string") {
      ...
      if (jsonMatch) {
        ...
        if (parsed.success) {
          const toolCallData = parsed.value as Record<string, unknown>;
    ...
    if (delta.tool_calls && Array.isArray(delta.tool_calls)) {
      ...
      for (const toolCallDelta of toolCallDeltas) {
        ...
        const toolCallId = toolCallDelta.id as string;
        const functionDelta = toolCallDelta.function as Record<string, unknown>;
        ...
        if (functionDelta?.name) {
          const toolName = functionDelta.name as string;
          const initialArgs = (functionDelta.arguments as string) || "";
        ...
        if (typeof func.arguments === "string") {
          if (func.arguments.trim()) {
            ...
            if (parseResult.success) {
              args = parseResult.value as Record<string, unknown>;
            } else {
              console.warn(
