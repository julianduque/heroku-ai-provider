...
/**
 * Request body interface for Heroku reranking API (Cohere-compatible).
 * @internal
 */
interface HerokuRerankingRequestBody extends Record<string, unknown> {
  model: string;
  query: string;
  documents: string[];
  top_n?: number;
...
/**
 * Response structure from Heroku reranking API (Cohere-compatible).
 * @internal
 */
interface HerokuRerankingResponse {
  id?: string;
  results: Array<{
    index: number;
 ...
 * ```
 */
export class HerokuRerankingModel implements RerankingModelV3 {
  ...
  async doRerank(options: RerankingModelV3CallOptions): Promise<{
    ...
    if (options.documents.type === "text") {
      ...
      warnings.push({
        type: "other",
        message:
          "Object documents are being converted to JSON strings. For optimal results, consider using text documents.",
      });
      documentStrings = options.documents.values.map((doc: JSONObject) =>
        JSON.stringify(doc),
      );
    }
    ...
    // Build request body
    const body: HerokuRerankingRequestBody = {
      model: this.model,
      query: options.query,
    ...
    if (
      ...
      warnings.push({
        type: "other",
        message:
          "providerOptions are not supported by HerokuRerankingModel and will be ignored.",
      });
    }
