...
interface HerokuImageGenerationRequestBody extends Record<string, unknown> {
  model: string;
  prompt: string;
  n?: number;
  size?: string;
...
interface HerokuImageGenerationResponse {
  created?: number;
  data: Array<{
    b64_json?: string;
    url?: string;
 ...
 * ```
 */
export class HerokuImageModel implements ImageModelV2 {
  ...
  private validateConstructorParameters(
    ...
    try {
      ...
      if (error instanceof Error && error.name === "TypeError") {
        throw createValidationError(
          `baseUrl is not a valid URL: ${error.message}`,
          "baseUrl",
          baseUrl,
        );
      }
  ...
  async doGenerate(options: {
    ...
    if (options.aspectRatio) {
      warnings.push({
        ...
        setting: "aspectRatio",
        details:
          "Heroku image generation currently does not support custom aspect ratios. Request will continue using provider defaults.",
      });
    }
    ...
    if (options.seed !== undefined) {
      warnings.push({
        ...
        setting: "seed",
        details:
          "Heroku image generation currently does not support deterministic seeds.",
      });
    }
    ...
    if (options.size) {
      ...
      if (!sizePattern.test(options.size)) {
        throw createValidationError(
          "size must be in the format {width}x{height}",
          "size",
          options.size,
        );
      }
    ...
    const baseRequest: HerokuImageGenerationRequestBody = {
      model: this.model,
    ...
    )) as HerokuImageGenerationResponse;

    return this.handleResponse(response, warnings);
  }

  private handleResponse(
    response: HerokuImageGenerationResponse,
    ...
    const metadata: ImageModelV2ProviderMetadata = {
      heroku: {
