 ...
 */
export const HTTP_STATUS_TO_ERROR_TYPE: Record<number, HerokuErrorType> = {
  // 4xx Client Errors
 ...
 */
export const ERROR_METADATA: Record<HerokuErrorType, ErrorMetadata> = {
  [HerokuErrorType.AUTHENTICATION_ERROR]: {
    ...
    technicalMessage:
      "Invalid or missing API key for Heroku API authentication",
    recoverySuggestions: [
  ...
  [HerokuErrorType.AUTHORIZATION_ERROR]: {
    ...
    retryable: false,
    userMessage:
      "Access denied. Your account does not have permission to use this feature.",
  ...
  [HerokuErrorType.CONCURRENT_REQUESTS_LIMIT]: {
    ...
    retryable: true,
    userMessage:
      "Too many concurrent requests. Please reduce request frequency.",
 ...
 */
export function getErrorMetadata(errorType: HerokuErrorType): ErrorMetadata {
  return (
    ERROR_METADATA[errorType] || ERROR_METADATA[HerokuErrorType.UNKNOWN_ERROR]
  );
}
 ...
 */
export function isRetryableError(errorType: HerokuErrorType): boolean {
 ...
 */
export function getErrorSeverity(errorType: HerokuErrorType): ErrorSeverity {
 ...
 */
export function getErrorCategory(errorType: HerokuErrorType): ErrorCategory {
