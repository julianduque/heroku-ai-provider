 ...
 */
export function mapHerokuError(
  status: number,
  errorData: unknown,
  url = "",
  ...
  const errorType =
    HTTP_STATUS_TO_ERROR_TYPE[status] || HerokuErrorType.UNKNOWN_ERROR;
 ...
 */
function parseErrorResponse(errorData: unknown): HerokuErrorResponse {
...
function extractErrorMessage(
  errorResponse: HerokuErrorResponse,
 ...
 */
export function createUserFriendlyErrorMessage(error: APICallError): string {
 ...
 */
export function shouldRetryError(error: APICallError): boolean {
  const errorData = error.data as ErrorDataWithType;
  const errorType = errorData?.errorType;
 ...
 */
export function mapStreamError(error: Error, url: string): APICallError {
