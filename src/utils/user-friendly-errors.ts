...
interface ErrorDataWithType {
  errorType?: HerokuErrorType;
  [key: string]: unknown;
}
...
export function createUserFriendlyError(
  ...
  const errorData = error.data as ErrorDataWithType;
 ...
 */
function createGenericUserFriendlyError(
  error: APICallError,
): UserFriendlyError {
  const statusCode = error.statusCode;

  // Determine category based on status code
  let category: ErrorCategory = ErrorCategory.CLIENT_ERROR;
 ...
 */
function getEstimatedResolutionTime(
  errorType: HerokuErrorType,
  severity: ErrorSeverity,
): string {
  switch (severity) {
 ...
 */
export function formatUserFriendlyError(userError: UserFriendlyError): string {
 ...
 */
export function createSimpleErrorMessage(error: APICallError): string {
 ...
 */
export function createDetailedErrorReport(error: APICallError): string {
  const userError = createUserFriendlyError(error);
 ...
 */
export function isConfigurationError(error: APICallError): boolean {
  const errorData = error.data as ErrorDataWithType;
  const errorType = errorData?.errorType;
 ...
 */
export function isTemporaryServiceError(error: APICallError): boolean {
  const errorData = error.data as ErrorDataWithType;
  const errorType = errorData?.errorType;
 ...
 */
export function getContextualHelp(error: APICallError): string[] {
  const errorData = error.data as ErrorDataWithType;
  const errorType = errorData?.errorType;
